<template>
  <div class="flex-1 max-w-2xl mx-auto flex flex-col items-center justify-center w-full px-4">
    <!-- Song info -->
    ...
    <div class="w-full">
      ...
      <div class="flex items-center space-x-2 mb-1">
        ...
        <!-- Progress bar -->
        <div ref="progressBarRef" class="relative flex-1 h-2 bg-gray-700 rounded cursor-pointer group"
          @click="handleProgressBarClick" @mousemove="handleProgressBarHover" @mouseleave="handleProgressBarLeave">
          <!-- Progress indicator -->
          <div class="absolute top-0 left-0 h-full bg-yellow-400 rounded" :style="{ width: `${progressPercentage}%` }">
          </div>

          <!-- Hover position indicator -->
          <div v-if="seekPosition !== null" class="absolute top-0 h-full bg-yellow-200 opacity-30 rounded"
            :style="{ left: `${hoverPercentage}%`, width: '2px' }"></div>

          <!-- Time tooltip on hover -->
          <div v-if="formattedSeekPosition"
          ...
          <!-- Loading indicator -->
          <div v-if="isLoading" class="absolute top-0 left-0 h-full w-full overflow-hidden">
            <div
      ...
      <!-- Playback controls -->
      <div class="flex items-center justify-center space-x-4 -translate-y-[5px]">
        <!-- Previous button -->
        <button
          ...
          tabindex="0" aria-label="Previous song" @click="playPrevious">
        ...
        <button
          ...
          tabindex="0" :aria-label="isPlaying ? 'Pause' : 'Play'" @click="togglePlay">
        ...
        <button
          class="text-gray-300 hover:text-white p-0 rounded-full hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-yellow-400"
          tabindex="0" aria-label="Next song" @click="playNext">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path
        ...
        <button v-if="currentSong"
          ...
          tabindex="0" aria-label="Toggle speed controls" :aria-expanded="speedPanelVisible" title="Playback speed"
          @click="toggleSpeedPanel" @keydown.enter="toggleSpeedPanel" @keydown.space.prevent="toggleSpeedPanel">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
          <span class="font-medium">{{ formattedPlaybackSpeed }}</span>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 ml-1 transition-transform duration-150"
            :class="speedPanelVisible ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button v-if="currentSong"
          class="text-gray-300 hover:text-yellow-400 p-1 rounded bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-400 ml-1 text-xs flex items-center transition-colors duration-150"
          :class="{ 'bg-gray-600 text-yellow-400': enableCrossfade }" tabindex="0"
          aria-label="Toggle crossfade controls" :aria-expanded="crossfadePanelVisible" title="Crossfade settings"
          @click="toggleCrossfadePanel" @keydown.enter="toggleCrossfadePanel"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 ml-1 transition-transform duration-150"
            :class="crossfadePanelVisible ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
          </svg>
        </button>
      </div>
...
const speedPanelStateHandler = ((event: CustomEvent) => {
  speedPanelVisible.value = event.detail.visible;
}) as EventListener;

const crossfadePanelStateHandler = ((event: CustomEvent) => {
  crossfadePanelVisible.value = event.detail.visible;
}) as EventListener;

// Listen for panel close events from other components
onMounted(() => {
  window.addEventListener("speed-panel-state-change", speedPanelStateHandler);
  window.addEventListener("crossfade-panel-state-change", crossfadePanelStateHandler);
});

// Remove event listeners on unmount
onUnmounted(() => {
  window.removeEventListener("speed-panel-state-change", speedPanelStateHandler);
  window.removeEventListener("crossfade-panel-state-change", crossfadePanelStateHandler);
});
